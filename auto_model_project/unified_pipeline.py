...
def _apply_projection_formulas(final_path: Path) -> None:
    ...
    # Force assumptions block to absolute values (when numeric) and percent format
    ...
    # Safety pass: ensure projected Capex % rows link to assumption (e.g., H29 = Q9)
    if capex_pct_row and max_actual_year:
        for year, col_idx in year_map.items():
            if year > max_actual_year:
                cell = ws.cell(row=capex_pct_row, column=col_idx)
                cell.value = f"={ASSUMP['capex']}"
                cell.number_format = "0%;(0%)"
